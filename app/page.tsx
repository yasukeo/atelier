...
import { t, tObj, tFn } from '@/lib/i18n'
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 pt-16 pb-14 flex flex-col gap-10 lg:flex-row lg:items-center">
          <div className="flex-1 max-w-2xl">
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-[#6B2D2D]">
              {t('home.hero.title')}
            </h1>
            <p className="mt-6 text-base md:text-lg text-[#8B7355] leading-relaxed">
      ...
      {/* Featured paintings */}
      <section className="py-14 border-t border-[#DCD9CC] bg-[#F7F5F0]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
            <h2 className="text-2xl font-semibold tracking-tight text-[#2D2A26]">{t('home.featured.title')}</h2>
      ...
      <section className="py-20 bg-[#E9E7DB] border-t border-[#D8D5C8]">
          ...
          {(['quality','customization','trust'] as const).map(key => {
            ...
            return (
              <div key={key} className="space-y-3">
                <div className="text-[#D97706] text-sm font-medium tracking-wide uppercase">{base.kicker}</div>
                <h3 className="text-lg font-semibold leading-snug text-[#2D2A26]">{base.title}</h3>
                <p className="text-sm text-[#8B7355] leading-relaxed">{base.text}</p>
              </div>
            )
          })}
        </div>
